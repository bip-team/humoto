        ...
        */
        ...
        for (std::size_t i = 0;; ++i)
        {
            ...
            if (mg.updateAndShift(  motion_parameters,
                                    model,
                                    mg_parameters.subsampling_time_ms_) != humoto::ControlProblemStatus::OK)
            {
                break;
            }
            ...
            model_state.log(logger, prefix, "current_state");
            //logger.log(humoto::LogEntryName(prefix).add("current_cop"), model.getCoP(model_state));


            // extract next model state from the solution and update model
            model_state = mg.getModelState(model, mg_parameters.subsampling_time_ms_);
            model.updateState(model_state);
            //model_state.log(logger, prefix, "next_state");
            //logger.log(humoto::LogEntryName(prefix).add("next_cop"), model.getCoP(model_state));
        }
    }
