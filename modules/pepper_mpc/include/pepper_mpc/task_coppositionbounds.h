...
namespace humoto
{
    ...
    {
        /**
         * @brief CoP position bounding task
         */
        class HUMOTO_LOCAL TaskCoPPositionBounds: public humoto::TaskALU
        {
            public:
                TaskCoPPositionBounds() : TaskALU("TaskCoPPositionBounds", 0.0)
                {
                }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            HUMOTO_ASSERT(  (getActualActiveSet().size() == num_ctr),
                                            "The number of CoP constraints is not supposed to change.");
